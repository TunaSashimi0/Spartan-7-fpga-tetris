...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE code = XSpi_Transfer(&SpiInstance, buffer, NULL, 2);

	if(code != 0) {
		xil_printf("Single host register read error!");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	BYTE code = XSpi_Transfer(&SpiInstance, buffer, NULL, nbytes + 1);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	BYTE code = XSpi_Transfer(&SpiInstance, buffer, buffer, 2);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	BYTE code = XSpi_Transfer(&SpiInstance, buffer, buffer, nbytes + 1);
